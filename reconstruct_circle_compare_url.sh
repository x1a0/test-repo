...
# This is borrowed from https://github.com/iynere/compare-url/blob/master/src/commands/reconstruct.yml
# with some small changes to make it work for our pipeline.

# Required ENV vars that are not set by CircleCI:
...
check_if_branch_is_new () {
	...
	# clean up
	if [[ ${CIRCLE_COMPARE_URL_DEBUG} == false ]]; then
...
do
	...
	if [[ $BRANCH_IS_NEW == true ]]; then
		...
		if [[ ! $(grep "\"workflow_id\" : \"$CIRCLE_WORKFLOW_ID\"" JOB_OUTPUT) && \
			...
			# (we have already ruled out that this is a brand-new branch)
